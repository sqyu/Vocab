...
def number(i):
	"""
	"""
	f = open(pathToVocab + "Word list with no meaning/Petit Livre Rouge/" + str(i)+".txt")
	j = 0
	for s in f:
		if (s != '\n') & (not isInt(s.rstrip('\n'))):
			j += 1
...
def addWordListToJap(i):
	...
	f.close()
	g = open(pathToVocab + 'Word list with no meaning/Petit Livre Rouge/' + str(i) + ' 2.txt', 'w')
	g.write(" || ".join(wordlist) + '\n' + ''.join(words))
	g.close()
	return

def addWordListToChi(i):
	f = open(pathToVocab + 'Word list with no meaning/Petit Livre Rouge/' + str(i) + ' 2.txt')
	wordlist = f.readline()
	gcn = codecs.open(pathToVocab + 'Word lists/Petit Livre Rouge/Other/2 After adding page number before adding word list/' + str(i) + ' zh-cn.txt', 'r', 'utf-16')
	hcn = codecs.open(pathToVocab + 'Word lists/Petit Livre Rouge/' + str(i) + ' zh-cn.txt', 'w', 'utf-16')
	ghk = codecs.open(pathToVocab + 'Word lists/Petit Livre Rouge/Other/2 After adding page number before adding word list/' + str(i) + ' zh-hk.txt', 'r', 'utf-16')
	hhk = codecs.open(pathToVocab + 'Word lists/Petit Livre Rouge/' + str(i) + ' zh-hk.txt', 'w', 'utf-16')
	hcn.write(wordlist)
	hhk.write(wordlist)
	for s in gcn:
		hcn.write(s)
...
def formatting(i):
	...
	"""
	f = codecs.open(pathToVocab + 'Word lists/Petit Livre Rouge/0 Unformatted Chinese/' + str(i) + ' zh-cn.txt', 'r', 'utf-16')
	g = codecs.open(pathToVocab + 'Word lists/Petit Livre Rouge/' + str(i) + ' zh-cn.txt', 'w', 'utf-16')
	for s in f:
...
def unifyWordList(i):
	...
	"""
	f = codecs.open(pathToVocab + 'Word lists/Petit Livre Rouge/' + str(i) + ' zh-cn.txt', 'r', 'utf-16')
	g = codecs.open(pathToVocab + 'Word list with no meaning/Petit Livre Rouge/' + str(i) + '.txt', 'r', 'utf-8')
	wordsf = []
	wordsg = []
	for s in f:
	...
	if len(wordsf) != len(wordsg):
		print("{0} words in Chinese but {1} words in my word list.".format(len(wordsf), len(wordsg)))
	return

def copyAndChangeNameToHK(i):
	shutil.copy(pathToVocab + 'Word lists/Petit Livre Rouge/' + str(i) + ' zh-cn.txt', pathToVocab + 'Word lists/Petit Livre Rouge/' + str(i) + ' zh-hk.txt')
	return
...
def addPageNumber(i):
	...
	f.close()
	g.close()
	f = codecs.open(pathToVocab + 'Word lists/Petit Livre Rouge/' + str(i) + ' zh-cn.txt', 'w', 'utf-16')
	f.write('\n'.join(toPrint))
	return
...
def formatPOS(i):
	...
	for file in files:
		f = codecs.open(pathToVocab + 'Word lists/Petit Livre Rouge/Other/3 After adding word list before formatting part of speech and splitting meanings/' + str(i) + file, 'r', 'utf-16')
		...
		for s in f:
			...
			for j, meaning in enumerate(meaningsbypos): # Meanings with the same part of speech
				if ";" in meaning: ### "；" recognized as \uff1b, and meaningsbypos when printed only show u'something'
...
def formatIPA(ipa):
	if "KK" in ipa:
		ipa = ipa.replace("(", "").replace(")", "").replace("KK", "") # Keep the segment inside the parentheses
	elif "(" in ipa:
		for i, c in enumerate(ipa):
			if c == "(":
				...
				right = i
		ipa = ipa[0:left] + ipa[right + 1 : len(ipa)] # Remove everything inside the parentheses and the parenthese themselves
	return ("[" + ipa.replace("|", "").replace(" FOR", "] for").replace("/ ","/ [").replace(" OR ", "] / [") + "]").replace(" AND ", "] & [").replace(".]", ".").replace("[ ", "[").replace("[ ", "[").replace(" ]", "]").replace(" ]", "]").replace("\'", "ˈ").replace("\"", "ˌ").replace("dʒ","ʤ").replace("E", "ə").replace("I","ɪ").replace("DG", "ʤ").replace("U","ʊ").replace("NG","ŋ").replace(":", "ː").replace("SH", "ʃ")
...
def addIPA(i, c = 'PLR'):
	...
	#
	if os.path.exists(pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + ".txt"):
		g = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + ".txt")
		s = "".join(g.readlines())
		for word in dictionary:
			if not word in s:
				print("Word {0} not in IPA'ed list!!\n".format(word)) # Sanity check if any word is missing
		print("File already exists.\n")
		return
	# If file already exists
	#
	if os.path.exists(pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + " 0.txt"): # If partially imported from Ja, or inputed partially before and quitted, reload
		f = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + " 0.txt")
		fileExists = True
	else:
		f = open(pathToVocab + "Word list with no meaning/Petit Livre Rouge/" + str(i) + ".txt")
	g = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + ".txt", "w")
	mispronounced = {}
	noMoreIPA = False
	previousWord = ""
	previousIPA = ""
	for s in f:
		...
		if (s == '\n') | (isInt(s.rstrip('\n'))):
			...
			if ((not fileExists) or (len(s) == 1) or (not "[" in s[len(s)-1])) and (not noMoreIPA):
				...
				if (len(s) != 1) and (not "[" in s[len(s)-1]): # If some word behind the point of quit has Chinese meaning but no IPA, only print the word
					g.write(s[0] + '\n')
				else:
					g.write(" **".join(s) + '\n')
	f.close()
	...
	if mispronounced != {}:
		...
		e = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + " 2.txt", "w")
		g = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + ".txt")
		for s in g:
			word = s.rstrip('\n').split(" **")[0]
		...
		g.close()
		e.close()
		os.rename(pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + " 2.txt", pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + ".txt")
	g = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + ".txt")
	s = "".join(g.readlines())
	for word in dictionary:
		if not word in s:
			print("Word {0} not in IPA'ed list!!\n".format(word)) # Check if any word is missing
	g.close()
	if noMoreIPA:
		os.rename(pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + ".txt", pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + " 0.txt")
	return

def IPAFromJa(i):
	assert i in [3,5,11,12,13,14]
	f = open(pathToVocab + "Word lists/Petit Livre Rouge/" + str(i) + " ja.txt")
	g = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + " 0.txt", "w")
	for s in f:
		s = s.rstrip('\n').split(" **")
		if len(s) == 3:
...
def combineIPA(i, proofread = False, c = 'z'): # FOR UNICODE
	...
	for c in l:
		if "zh" in c:
			try:
				f = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " " + c + ".txt")
				g = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/FINAL LISTS/" + str(i) + " " + c + ".txt", 'w')
			except IOError:
				f = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/7 Initially proofread/" + str(i) + " " + c + ".txt")
				g = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/7 + IPA/" + str(i) + " " + c + ".txt", 'w')
		else:
			f = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/JA/" + str(i) + " ja.txt")
		h = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/5 Lists with pronunciation/" + str(i) + ".txt")
		if c == "ja":
			f.readline()
			f.readline()
		while True:
...
def combineDifficultWordList(i):
	for c in ["zh-cn", "zh-hk"]:
		try:
			f = open(pathToVocab + "/Word lists/Petit Livre Rouge/Other/FINAL LISTS/" + str(i) + " " + c + ".txt")
			print(str(i) + ": " + c + " from FINAL LISTS.\n")
		except IOError:
			f = open(pathToVocab + "/Word lists/Petit Livre Rouge/Other/7 + IPA/" + str(i) + " " + c + ".txt")
			print(str(i) + ": From 7 + IPA.\n")
		g = open(pathToVocab + "/Word lists/Petit Livre Rouge/Other/-1 Difficult Words/" + str(i) + ".txt")
		h = open(pathToVocab + "/Word lists/Petit Livre Rouge/" + str(i) + " " + c + ".txt", "w")
...
def formatWord3000(s):
	return s.replace("\\\\", "SLASH").replace("\\", "\\\\").replace("SLASH", "\\\\").replace("\"", "").replace("N/A", "not applicable").replace("B", "\\" + "033[1m").replace("N", "\\" + "033[0m").replace("not applicable", "N/A")
...
def input3000helper(s, meaning = None):
	...
	while True:
		...
		meanings.append(meaning + " \\ " + description + " \\ " + synonym + " \\ " + antonym)
		i += 1
	if not quit:
...
def input3000():
	if not os.path.isfile(pathToVocab + "Word lists/Trois Mille/Backup/3000.txt"):
		shutil.copy(pathToVocab + "Word lists/Trois Mille/3000.txt", pathToVocab + "Word lists/Trois Mille/Backup/3000.txt")
	else:
		i = 2
		while os.path.isfile(pathToVocab + "Word lists/Trois Mille/Backup/3000 " + str(i) + ".txt"):
			i += 1
		shutil.copy(pathToVocab + "Word lists/Trois Mille/3000.txt", pathToVocab + "Word lists/Trois Mille/Backup/3000 " + str(i) + ".txt")
	...
	for s in h:
		assert(s.rstrip('\r\n').rstrip(' ') in wordlist)
	os.rename(pathToVocab + "Word lists/Trois Mille/3000 0.txt", pathToVocab + "Word lists/Trois Mille/3000.txt")
	return
...
def removeEmptyMeaningsAndDoubleSpaces(i):
	POS = ["n.", "v.", "adj.", "adv."]
	for c in ["cn", "hk"]:
		f = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/4 After formatting meanings by part of speech and seperating different meanings by semicolon/" + str(i) + " zh-" + c + ".txt")
		g = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/6 Empty meanings and double spaces removed/" + str(i) + " zh-" + c + ".txt", 'w')
		for s in f:
			for pos in POS:
				if ("%% " + pos + "\n" in s) or ("%% " + pos + " \n" in s):
					print(s)
...
def removeIPA(i): # Removes the IPA mistakenly included in lists in 7 Initially proofread
	for c in ["zh-cn", "zh-hk"]:
		f = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/7 Initially proofread/" + str(i) + " " + c + ".txt")
		g = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/7 Initially proofread/" + str(i) + " " + c + " 2.txt", 'w')
		for s in f:
			if len(s.split(" ** ")) > 1:
				s = s.split(" ** ")
				s[1] = "N/A"
		...
		f.close()
		g.close()
		os.rename(pathToVocab + "Word lists/Petit Livre Rouge/Other/7 Initially proofread/" + str(i) + " " + c + " 2.txt", pathToVocab + "Word lists/Petit Livre Rouge/Other/7 Initially proofread/" + str(i) + " " + c + ".txt")
	return
...
def instructions():
	...
	for file in reads + writes:
		file.close()
	os.rename(pathToSystemFiles + "instructions_en 2.txt", pathToSystemFiles + "instructions_en.txt")
	os.rename(pathToSystemFiles + "instructions_hans 2.txt", pathToSystemFiles + "instructions_hans.txt")
	os.rename(pathToSystemFiles + "instructions_hant 2.txt", pathToSystemFiles + "instructions_hant.txt")
	os.rename(pathToSystemFiles + "instructions_ja 2.txt", pathToSystemFiles + "instructions_ja.txt")
	return
...
def formatChinese(word, my_input, noInput = False):
	...
	for m in output.split(" %% "):
		...
		if ("adj" in m and m[len(m) - 1] == "\xb0") or ("adv" in m and m[len(m) - 1] == "\x84"):
			print("\nWARNING: Check \"{0}\". \"的\" or \"地\"?\n".format(m))
	return word + " ** N/A ** " + output

def pr(i): # proofread
	if not os.path.exists(pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " 0.txt"):
		f = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/7 Initially proofread/" + str(i) + " zh-cn.txt")
		g = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " 0.txt", 'w')
		g.write(f.readline() + f.readline() + f.readline().rstrip("\n") + "STARTED" + "\n" + "".join(f.readlines()))
		f.close()
		g.close()
	f = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " 0.txt")
	g = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " zh-cn.txt", 'w')
	quitted = False
	started = False
	previousWord = None
	try:
			...
			if not quitted and started: # The line started now changed started to True
				if s == "\n" or isInt(s.rstrip("\n")):
					...
					if my_input.upper() == "":
						...
						f.close()
						g.close()
						os.rename(pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " zh-cn.txt", pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " 0.txt")
						f = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " 0.txt")
						g = open(pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " zh-cn.txt", 'w')
						for ss in f:
		...
		if quitted:
			os.rename(pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " zh-cn.txt", pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " 0.txt")
		elif os.path.exists(pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " zh-cn.txt"):
			shutil.copy(pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " zh-cn.txt", pathToVocab + "Word lists/Petit Livre Rouge/Other/8 Completely proofread/" + str(i) + " zh-hk.txt")
	except:

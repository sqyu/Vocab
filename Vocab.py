    ...
    if len(booksToChoose) == 1:  # If there is only one book
        ...
        for book in sorted(booksToChoose):
            printInst(
                "book2",
                (
                    ("REPLACE1", getInst("book" + Vars.acronym[book])),
                    ("REPLACE2", Vars.acronym[book]),
                ),
            )
        try:
            book = quitOrInput("\n").upper()  # Select book
            while book not in Vars.findAcronym:  # Book not found
                book = quitOrInput(
                    getInst(
                        "wrongBook",
                        rep="".join(
                            [
                                getInst("guillemetLeft") + b + getInst("guillemetRight")
                                for b in [
                                    getInst("book" + Vars.acronym[bo])
                                    for bo in sorted(booksToChoose)
                                ]
                            ]
                        ),
                    )
...
def chooseList(listsinthebook, choose_all, firstList):
    """
    chooseList(listsinthebook, choose_all, firstList):
    Returns the list number (int), and the bool choose_all
    Notes: if there are at least two lists in the book, list number + "S" sets "choose_all" to
     False,
        and "A" sets "choose_all" to True
    ...
    if len(listsinthebook) == 1:  # If there is only one list in the giveen list
        ...
        return theList, choose_all
    else:
        theList = inpInst(
            "chooseList" if firstList else "chooseMoreLists"
        ).upper()  # Select list; QuitException handled in createDictionary()
        while True:  # If the user did not enter a number
            if not isInt(theList):
                if (
                    "S" in theList
                ):  # List number + "S" to indicate that read the list of selected words only
                    # ("S" stands for "select")
                    theList = theList.replace("S", "")
                    choose_all = False
                elif (
                    "A" in theList
                ):  # List number + "A" to indicate that read the full list ("A" stands for "all")
                    theList = theList.replace("A", "")
                    choose_all = True
                elif (not firstList) and theList == "N":
                    return None
                else:
                    theList = inpInst(
                        "enterANumber" if firstList else "chooseMoreListsWrong",
                        handleQuit=False,
                    ).upper()
            if isInt(theList):
                theList = int(theList)
...
def addWordsFromLists(
    allWordsFromLists, book, lists
):  # Helper function of checkAllWordsInLists
    """
...
def checkAllWordsInLists(
    wordList, book, lists
):  # Check if all words in the record or the difficult word list are in the word lists
    """
    ...
    if isinstance(book, list):  # For "similar words" mode only
        for books in book:
            addWordsFromLists(
                allWordsFromLists, books, Vars.listNumber[Vars.lang][books]
            )
    else:
        addWordsFromLists(allWordsFromLists, book, lists)
    for word in wordList:  # Check if all the words are in the lists
        if word not in allWordsFromLists:
            if len(lists) == 1:
                printInst(
                    "warning",
                    add=": "
                    + word
                    + getInst("notInList").replace("REPLACE", str(lists[0])),
                )
            elif (not isinstance(book, list)) and sorted(lists) == sorted(
                Vars.listNumber[Vars.lang][book]
            ):
                printInst(
                    "warning",
                    add=": "
                    + word
                    + getInst("notInBook").replace(
                        "REPLACE",
                        getInst("guillemetLeft")
                        + getInst("book" + Vars.acronym[book])
                        + getInst("guillemetRight"),
                    ),
                )
                    ...
                    + getInst(
                        ...
                        rep=" & ".join(
                            [
                                ...
                                + getInst("book" + Vars.acronym[book])
                                + getInst("guillemetRight")
                                for book in Vars.availableBooks
                            ]
                        ),
                    ),
                )
                ...
                printInst(
                    ...
                    add=": "
                    + word
                    + getInst("notInLists", rep=" & ".join(map(str, lists))),
                )
...
def loadWords(choose_all, book, comments, Dictionary, mode, theList, wordList):
    """
    loadWords(choose_all, book, comments, Dictionary, mode, theList, wordList):
...
def loadConj(choose_all, book, comments, Dictionary, mode, theList, conjList):
    """
    loadConj(choose_all, book, comments, Dictionary, mode, theList, conjList):
    Adds Conjs to Dictionary and word strings to conjList

    Parameters
    __________
    choose_all: bool
    Whether add choose_all words or not
    ...
    """
    bookList = os.path.join(
        Vars.wordLists_path, book, "Lists", str(theList) + " conj.txt"
    )
    f = open(bookList)  # Read Only
    difficultWordFile = os.path.join(
        Vars.wordLists_path, book, "Difficult Words", str(theList) + ".txt"
        ...
        if "**" in s:
            try:
                ...
                if add_this_word(conj_obj.infinitive):
                    ...
                    conj_obj.meanings = (
                        "\n".join(worddic[conj_obj.infinitive].meanings)
                        if conj_obj.infinitive in worddic
                        else ""
                    )
                ...
                print(
                    "Failed to load conjugations for %s: %s." % (s, e)
                )  # TODO: INSTRUCTIONS PENDING
    f.close()
    return


def read_conj(Dictionary, mode, defaultAll=True):
    ...
    if book is None:
        return
    while True:
        choose_all = defaultAll
        try:
            theListAndAll = chooseList(conjNumber[book], choose_all, firstList)
        except QuitException:
            return
        if theListAndAll is None:
            break
        theList, choose_all = theListAndAll
        theList = int(theList)
        lists.append(theList)
        alls.append(choose_all)
    ...
    if isinstance(book, list):
        for books in book:
            ...
            for theList in Vars.listNumber[Vars.lang][books]:
                loadWords(
                    False, books, comments, Dictionary[books], mode, theList, wordList
                )
    else:
...
def createDictionary(
    Dictionary, conj, readFromRecord=None, allLists=None, defaultAll=True
):
    """
    createDictionary(Dictionary, wordList):
    Creates a dictionary by asking the user to choose the book and the list, and returns the word
     list and the list composed of full book name and list numbers (integers)
    ...
    Whether read all words from the list BY DEFAULT or not (can be changed by user input through
     chooseList())
    """
    wordList = []
    book = ""
    ...
    if mode == 1:
        ...
        while True:
            choose_all = defaultAll
            try:
                theListAndAll = chooseList(list_of_books[book], choose_all, firstList)
            except QuitException:
                return
            if theListAndAll is None:
                break
    ...
    if mode == 3:
        ...
        alls = [defaultAll] * len(
            lists
        )  # Not necessarily include all words even if all lists are loaded
    if isinstance(book, list):
        for books in book:
            comments = loadComments(books)
            Dictionary[books] = {}
        ...
        for index, theList in enumerate(lists):
            if conj:
                loadConj(
                    alls[index], book, comments, Dictionary, mode, theList, wordList
                )
            else:
                loadWords(
                    alls[index], book, comments, Dictionary, mode, theList, wordList
...
def describeList(
    Dictionary, wordListAndNames, learnMode, rand, record, readFromRecord=False
):
    """
    ...
    try:
        for wi, word in enumerate(wordList):
            ...
            if s != "":
                ...
                if Word.markWordorNot(s):
                    ...
                    if previousWord in Dictionary:  # If not in "similar words" mode
                        comments[previousWord] = inpInst(
                            "addComments", rep=previousWord
                        )
                        print()
                else:
            ...
            ):  # If Dictionary has sub-dictionaries, in "silimar words" mode
                Words = [
                    Dictionary[dic][word]
                    for dic in sorted(Dictionary)
                    if word in Dictionary[dic]
                ]
                if len(Words) > 1:
                    WordsDics = [
                        dic for dic in sorted(Dictionary) if word in Dictionary[dic]
                    ]
            else:
            ...
            for Wi, Word_obj in enumerate(Words):
                if len(Words) > 1:
                    printInst(
                        "findAWordMeaningsIn",
                        rep=getInst("book" + Vars.acronym[WordsDics[Wi]]),
                    )
                s = Word.describeWord(
                    word, Word_obj, learnMode, [wi + 1, numberofwords]
                )  # "QUIT" for quit, "MARK" for mark, "" for nothing
                if Word.markWordorNot(s):
        ...
        if difficultWordsInput != "":
            ...
            for wi, word in enumerate(difficultWordsInput):
                while word not in wordList:
                    reword = input(
                        word + " " + getInst("wrongDifficultWordsInput") + "\n"
                    ).lower()  # ignores quit
                    if reword != "ab":
            ...
            if (
                abandoned != []
            ):  # Cannot change size of the dictionary during iteration; have to pop the words
                #  abandoned after iteration
    ...
    if difficultWords:  # If there is any difficult word
        ...
        print(
            ...
            + getInst("wordsInTotal2", rep=[("SPACE", "")]),
        )
        if not quit:
            try:
                ...
                for index, word in enumerate(difficultWords):
                    if (
                        word not in Dictionary
                    ):  # If Dictionary has sub-dictionaries, under "silimar words" mode
                        Words = [
                            Dictionary[dic][word]
                            for dic in sorted(Dictionary)
                            if word in Dictionary[dic]
                    ...
                    for index2, Word_obj in enumerate(Words):
                        if len(Words) > 1:
                            printInst(
                                "findAWordMeaningsIn",
                                rep=getInst("book" + Vars.acronym[WordsDics[index2]]),
                            )
                        Word.describeWord(
                            word, Word_obj, False, [index + 1, lenDifficultWords]
                        )  # learnMode is False, so QUIT ignored
                    _ = quitOrInput(None)
            except QuitException:
                pass
    writeTime(
        begin=False,
        timeBegan=timeBegan,
        mode=learnOrView + " mode " + readFromRecord * "(from record) ",
        names=name,
    )
    if comments != {}:
...
def randomAndRecord():
    """
    randomAndRecord():
    Returns two bools: true for the first one if random, and true for the second one if record.
     Or returns None if "quit" entered
    """
    try:
...
def recite(Dictionary, rnr, wordListAndNames, readFromRecord=False):
    ...
    try:
        for wi, word in enumerate(wordList):
            ...
            #   wi + 1, numberofwords, len(word), getInst("letters"))
            ...
            while (not comp_answers(user_input, word)) and (
                ...
                if user_input.upper() == "HINT":
                    ...
                    print(
                        "{0} {1}".format(len(word.replace("-", "")), getInst("letters"))
                    )
                    user_input = quitOrInput()
                    continue
                wrongtimes += 1
    ...
    if difficultWords:  # If there is any difficult word
        ...
        if not quit:
            try:
                ...
                for index, word in enumerate(difficultWords):
                    Word.describeWord(
                        word, Dictionary[word], False, [index + 1, lenDifficultWords]
                    )  # Does not raise QuitException since learnMode is False
                    _ = quitOrInput()
            except QuitException:
                pass
    ...
    writeTime(
        begin=False,
        timeBegan=timeBegan,
        mode="recite mode " + readFromRecord * "(from record) ",
        names=name,
    )
    return
...
def learnAndView(
    learnMode, rnr, Dictionary=None, wordListAndNames=None, readFromRecord=False
):
    """
    ...
    Must come together with wordListAndNames if a particular word list is predermined; cannot be
     non-None at the same time as readFromRecord

    wordListAndNames: list
    Word list and name.
    Must come together with Dictionary if a particular word list is predermined; cannot be
     non-None at the same time as readFromRecord
    ...
    """
    random, record = rnr
    assert (Dictionary and (wordListAndNames and wordListAndNames[0])) or (
        not Dictionary and not (wordListAndNames and wordListAndNames[0])
    )  # The dictionary and the first item in wordListAndNames must be both None or both not None
    #       TypeError would occur if only check wordListAndNames[0] when wordListAndNames=None
    if (
        not Dictionary
        and not (wordListAndNames and wordListAndNames[0])
        and not readFromRecord
    ):  # Word List not assigned: not read from record and not random 50 words
        Dictionary = {}
        wordListAndNames = createDictionary(
            Dictionary, conj=False, defaultAll=not learnMode
        )
...
def findAWord():
    ...
    if pre == "Y":
        ...
        dics = [
            {} for i in range(0, len(Vars.availableBooks))
        ]  # Cannot use [{}] * len(Vars.availableBooks) which would create pointers pointing to
        #       the same dic
        for index, dic in enumerate(dics):
            wordListAndNames = createDictionary(
                dic, conj=False, allLists=Vars.availableBooks[index]
            )
            wordList += wordListAndNames[0]
            books.append(wordListAndNames[1][0])
    ...
    try:
        while True:
            ...
            while word not in wordList:
                if word == "":
                    ...
                    if len(wordsContaining) > maxWordsContaining:
                        word_copy = word
                        word = inpInst(
                            "moreThanMANYResults", rep=str(maxWordsContaining)
                        ).lower()
                        if word == "":
                            print(
                                getInst("chooseOneWord1")
                                + " "
                                + word_copy
                                + " "
                                + getInst("chooseOneWord2")
                                + "\n"
            ...
            if word in wordList:  # If not quitted
                ...
                inDics = (
                    []
                )  # The indices of the dictionaries in list "dics" that contain the word
                for index, dic in enumerate(dics):
                    if word in dic:
                        inDics.append(index)
                print("".center(80, "*") + "\n")
                for dicIndex in inDics:
                    printInst(
                        "findAWordMeaningsIn",
                        rep=getInst("book" + Vars.acronym[books[dicIndex]]),
                    )
                    Word.describeWord(
                        word, dics[dicIndex][word], False
                    )  # Does not raise QuitException since learnMode is False
    except QuitException:
        writeTime(
            begin=False, timeBegan=timeBegan, mode="words found: ", names=wordsFound
        )
    return
        ...
        while True:
            ...
            if date == "T" or date == "TODAY":
                date = datetime.datetime.now(
                    tz.localize(datetime.datetime.now()).tzinfo
                ).strftime("%m/%d/%Y")
                continue
            if date == "Y" or date == "YESTERDAY":
                date = (
                    datetime.datetime.now(tz.localize(datetime.datetime.now()).tzinfo)
        ...
        while True:
            ...
            if is_conj is not None:
                createDictionary(
                    Dictionary,
                    conj=is_conj,
                    readFromRecord=wordListAndNames,
                    defaultAll=False,
                )
                if Dictionary == {}:
                    printInst("noWordsToStudy")
                ...
                if is_conj:
                    ...
                    learnAndView(
                        ...
                        rnr=rnr,
                        Dictionary=Dictionary,
                        wordListAndNames=wordListAndNames,
                        readFromRecord=True,
                    )
                elif s in ["R", "RECITE"]:
    ...
    try:
        ...
        while True:
            if user_input == "VIEW":
                ...
                user_input = inpInst("scheduleEnterDate").upper()
            elif not isInt(user_input.split("/")[0]) or not isInt(
                user_input.split("/")[1]
            ):
                user_input = inpInst("scheduleEnterDate").upper()
                continue
            elif (not int(user_input.split("/")[0]) in range(1, 13)) or (
                not int(user_input.split("/")[1])
                in dates[int(user_input.split("/")[0])]
            ):
                user_input = inpInst("dateNotValid").upper()
            elif action == "A" and user_input in sch:
                user_input = inpInst("dateAlreadyExists").upper()
            elif action == "C" and user_input not in sch:
...
def allWordsOrDifficultWordListForRandom50Words():
    ...
    try:
        if Vars.parameters["AllForRandom50Words"] != "A":
            ...
            r = inpInst(
                "allForRandom50Words", rep=Vars.parameters["Random50Words"]
            ).upper()
        while r not in ["Y", "N"]:
            r = inpInst("enterYOrN").upper()
        return r == "Y"
...
def random50Words():
    ...
    if a is None:
        ...
        wordList, names = createDictionary(
            Dictionary, conj=False, allLists=book, defaultAll=a
        )
    ...
    try:
        while True:
            s = inpInst(
                "promptRandom50Words", rep=Vars.parameters["Random50Words"]
            ).upper()
            if s in ["L", "LEARN"]:
                try:
                    rnr = randomAndRecord()
                ...
                learnAndView(
                    ...
                    rnr=rnr,
                    Dictionary=Dictionary,
                    wordListAndNames=(wordList[0:NUMBER], names),
                )
            elif s in ["R", "RECITE"]:
                try:
                    rnr = randomAndRecord()
                ...
                random.shuffle(wordList)
                printInst(
                    "random50WordsShuffleDone", rep=Vars.parameters["Random50Words"]
                )
    except QuitException:
        return
    return
        ...
        while True:
            ...
            if word != "r":  # For CHANGE RANDOM
                ...
                if found == []:
                    ...
                    createDictionary(
                        Dictionary, conj=False, readFromRecord=wordListAndNames
                    )
                    name = ["similar words to {0}".format(word)]
                    while True:
                        s = quitOrInput(
                            "\n" + getInst("promptSimilarWords2", rep=word)
                        ).upper()
                        if s == "A":
                            f.seek(0)
                            print("\n".join(f.readlines()))
...
def extend(book, theList, current):
    ...
    for w in add:
        if w in all_words:
            ...
            while w2 not in all_words:
                if w2.upper() == "AB":
                    break
                w2 = input(
                    w + " " + getInst("wrongDifficultWordsInput")
                )  # ignores quit
            if w2 in all_words:
                new.append(w2)
...
def chooseDifficultWordList():
    ...
    if theList is None:
        return
    difficultWordListPath = os.path.join(
        Vars.wordLists_path, book, "Difficult Words", str(theList) + ".txt"
    )
...
def addComments():
    ...
    while s != "quit":
        if s == "":
            ...
            continue
        elif s not in allWordsFromBook:
            print(
                s
                + getInst(
                    "notInBook",
                    rep=getInst("guillemetLeft")
                    + getInst("book" + Vars.acronym[book])
                    + getInst("guillemetRight")
                    + getInst("reenter"),
                )
...
def study():
    ...
    while True:
        ...
        if s in ["L", "LEARN"]:
            ...
            Dictionary = {}
            wordListAndNames = createDictionary(
                Dictionary, conj=False, defaultAll=False
            )
            if wordListAndNames is None:
                continue
...
if __name__ == "__main__":
    ...
    )  # Change current working directory to where this file is located
